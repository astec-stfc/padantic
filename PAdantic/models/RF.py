...
class PIDElement(IgnoreExtra):
    ...
    def validate_phase_weight_range(cls, v: Union[str, List]) -> PIDWeightRange:
        if isinstance(v, str):
            ...
            raise ValueError('phase_weight_range should be a string or a list of numbers')
...
llrffieldnames = ['klystron_forward', 'klystron_reverse', 'cavity_forward','cavity_reverse', 'cavity_probe']
    ...
    def from_CATAP(cls: Type[T], fields: dict) -> T:
        ...
        for name in llrffieldnames:
            if 'ONE_RECORD_'+str.upper(name)+'_POWER' in fields:
                ...
                inputs[str.upper(name)] = LLRFChannelIndex(power=fields[substr+'_POWER'], phase=fields[substr+'_PHASE'])
            for cav in cavitynames:
                if 'ONE_RECORD_'+str.upper(cav)+'_'+str.upper(name)+'_POWER' in fields:
                    subname = str.upper(cav)+'_'+str.upper(name)
                    substr = 'ONE_RECORD_'+subname
                    inputs[subname] = LLRFChannelIndex(power=fields[substr+'_POWER'], phase=fields[substr+'_PHASE'])
...
class LLRFTimings(LLRFTimingsBase):
    ...
    def from_CATAP(cls: Type[T], fields: dict) -> T:
        for name, prefix in zip(*[llrffieldnames,llrftimingsCATAPnames]):
            fields[name] = LLRFTiming(start=fields[prefix+'pow_mean_start_time'], end=fields[prefix+'pow_mean_end_time'])
...
class LLRFElement(IgnoreExtra):
    ...
    def _create_LLRFChannels_Model(self, fields: dict):
        ...
        LLRFChannels = create_model('LLRFChannels', **model_fields, __base__=LLRFChannelsBase)
